...
while True:
    ...
    lmLists = tracker.getPostion(frame, draw=False)
    ...
    # If hands are detected, extract fingertip positions
    if lmLists:
        for lmList in lmLists:
            signTipXs.append(lmList[8][1])
            signTipYs.append(lmList[8][2])
            thumbTipXs.append(lmList[4][1])
            thumbTipYs.append(lmList[4][2])

    for i in range(len(signTipXs)):
        signTipX, signTipY = signTipXs[i], signTipYs[i]
        thumbTipX, thumbTipY = thumbTipXs[i], thumbTipYs[i]
